"""Extract header / subheader / legend / footer from figure images - no AI.
...
bounding boxes, then classifies blocks by position and type size:
  ...
  subheader - the subtitle directly below the header
  legend    - swatch-marked series rows (markers OCR as dashes/bullets)
  footer    - bottom block (typically "Source: ..."), excluding the
              T&E logo mark

Used in three places:
- extract_figure_text.py CLI writes data/figure_text.csv with text and
  wrapped-line counts per block
- the check engine crops a figure to one part (figure_type column in
  TE_style_rules) before sending it to the AI
- the coded footer-length check in document health (max two lines)
"""
...
def classify(lines: list[Line], img_height: int, img_width: int) -> dict:
    if not lines:
        return {"header": [], "subheader": [], "legend": [], "footer": []}
...
# ---- part extraction for the check engine and coded checks ---------------

# figure_type values in TE_style_rules -> classify() block names
FIGURE_TYPE_TO_PART = {
    "header": "header",
...
async def extract_parts(image: Image.Image) -> dict:
    """OCR + classify one figure. Returns per part name:
    {"text": str, "lines": int, "bbox": (x0, y0, x1, y1) | None}."""
    ...
    for name, block in blocks.items():
        bbox = None
        if block:
            y0 = max(0, min(l.y for l in block) - CROP_PAD)
            y1 = min(image.height, max(l.bottom for l in block) + CROP_PAD)
            if name == "header":
                y0 = 0  # capture the title from the very top
            if name == "footer":
                y1 = image.height  # through to the bottom edge
            # full-width crops keep horizontal context (logo, alignment)
            bbox = (0, int(y0), image.width, int(y1))
        parts[name] = {"text": block_text(block), "lines": len(block), "bbox": bbox}
    return parts


# OCR is the slow step (~1s per figure) and several consumers ask about the
# same image in one run (layout checks, figure-part crops, the CLI) - cache
# per path for the process lifetime.
_PARTS_CACHE: dict[str, dict] = {}


def extract_parts_path(image_path: str | Path) -> dict:
    """Sync wrapper for one image file, cached per path."""
    key = str(image_path)
    if key not in _PARTS_CACHE:
        image = Image.open(image_path).convert("RGB")
        _PARTS_CACHE[key] = asyncio.run(extract_parts(image))
    return _PARTS_CACHE[key]


def crop_part(image_path: str | Path, figure_type: str, parts: dict) -> Image.Image | None:
    """Crop the figure to the part named by the rule's figure_type.
    Returns None when the part was not found on this figure."""
    part_name = FIGURE_TYPE_TO_PART.get(figure_type)
    if not part_name:
        return None
    bbox = parts.get(part_name, {}).get("bbox")
    if not bbox:
        return None
    return Image.open(image_path).convert("RGB").crop(bbox)
